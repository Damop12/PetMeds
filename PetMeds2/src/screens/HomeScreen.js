import {
  ...
  export default function HomeScreen() {
    ...
    const emojis = { perro: '🐕', gato: '🐈', pájaro: '🐦', loro: '🦜', conejo: '🐇', pez: '🐟', tortuga: '🐢', hamster: '🐹' };
    ...
    const handleEliminar = (id) => {
      Alert.alert('Eliminar mascota', '¿Estás seguro que querés eliminarla?', [
        ...
        { text: 'Eliminar', style: 'destructive', onPress: () => eliminarMascota(id) },
      ]);
    };
    ...
    return (
      <View style={[styles.container, { backgroundColor: t.fondo }]}>
        ...
        <View style={styles.toggleRow}>
          <TouchableOpacity
            style={[styles.toggleBtn, { backgroundColor: tema === 'minimal' ? t.boton : t.borde }]}
            onPress={() => setTema('minimal')}
          >
            <Text style={[styles.toggleTexto, { color: tema === 'minimal' ? '#fff' : t.textoSecundario }]}>
              Minimal
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.toggleBtn, { backgroundColor: tema === 'colorful' ? '#e94560' : t.borde }]}
            onPress={() => setTema('colorful')}
          >
            <Text style={[styles.toggleTexto, { color: tema === 'colorful' ? '#fff' : t.textoSecundario }]}>
              Color
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={[styles.vacio, { color: t.textoSecundario }]}>No hay mascotas todavía</Text>
          }
          renderItem={({ item, index }) => (
            <TouchableOpacity
              style={[styles.tarjeta, { backgroundColor: t.avatares[index % t.avatares.length], borderColor: t.borde }]}
              onPress={() => router.push({ pathname: '/medicamentos', params: { id: item.id, nombre: item.nombre } })}
            >
              <Text style={styles.avatar}>{getEmoji(item.tipo)}</Text>
              <View style={styles.tarjetaInfo}>
                <Text style={[styles.nombre, { color: tema === 'colorful' ? '#fff' : t.texto }]}>{item.nombre}</Text>
                <Text style={[styles.tipo, { color: tema === 'colorful' ? 'rgba(255,255,255,0.7)' : t.textoSecundario }]}>{item.tipo}</Text>
              </View>
